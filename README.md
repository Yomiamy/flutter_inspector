...
A **Show raw / Show concise** button in the detail view switches between the two forms, and the copy and share actions export whichever one is on screen, so a stack trace pasted into a ticket matches the one being discussed. The stored trace is never modified — normalization is a presentation-layer projection only.

The same helper is available programmatically:
